...
import { Sparkles, Loader, FileText, ChevronLeft, ChevronRight, RotateCw, RefreshCw } from "lucide-react";
      ...
      <div className="glass-card p-6 mb-8 dark:bg-gray-800 dark:border-gray-700">
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px]">
            ...
            <select
              value={selectedDoc}
              onChange={(e) => { setSelectedDoc(e.target.value); setError(null); setFlashcards([]); }}
              className="input-custom dark:bg-gray-700 dark:text-white dark:border-gray-600"
            >
              <option value="">Choose a document...</option>
              {documents.map((doc) => (
                <option key={doc._id} value={doc._id}>{doc.title} ({doc.pageCount} pages)</option>
              ))}
            </select>
          </div>
          <div className="w-32">
        ...
        {error && (
          <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-600 dark:text-red-400 text-sm flex items-center justify-between">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="text-red-400 hover:text-red-600 dark:hover:text-red-300">Dismiss</button>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col items-center">
          <div className="relative w-full max-w-lg aspect-[3/2] cursor-pointer perspective-1000" onClick={toggleFlip}>
            <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${flipped ? "rotate-y-180" : ""}`}>
              <div className="absolute inset-0 glass-card p-8 flex items-center justify-center backface-hidden dark:bg-gray-800 dark:border-gray-700">
                <p className="text-xl font-medium text-gray-800 dark:text-white text-center">
                  {flashcards[currentIndex]?.question || "No question"}
                </p>
              </div>
              <div className="absolute inset-0 glass-card p-8 flex items-center justify-center backface-hidden rotate-y-180 bg-purple-50 dark:bg-purple-900/30 border-purple-200 dark:border-purple-700">
          ...
          <div className="flex items-center gap-6 mt-8">
            <button onClick={prevCard} disabled={currentIndex === 0} className="p-3 rounded-full border border-gray-300 dark:border-gray-600 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
              <ChevronLeft className="h-6 w-6" />
            </button>
            <button onClick={toggleFlip} className="p-3 rounded-full bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 hover:bg-purple-200 dark:hover:bg-purple-800 transition">
              <RotateCw className="h-6 w-6" />
            </button>
            <button onClick={nextCard} disabled={currentIndex === flashcards.length - 1} className="p-3 rounded-full border border-gray-300 dark:border-gray-600 disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
              <ChevronRight className="h-6 w-6" />
            </button>
          </div>

          <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">{currentIndex + 1} / {flashcards.length}</div>
          <div className="w-full max-w-lg mt-2 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
            <div className="bg-purple-600 h-1.5 rounded-full transition-all duration-300" style={{ width: `${((currentIndex + 1) / flashcards.length) * 100}%` }} />
          </div>
          <div className="flex gap-4 mt-6">
            <button onClick={() => { setFlashcards([]); setCurrentIndex(0); setFlipped(false); setError(null); }} className="text-sm text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 flex items-center gap-1">
